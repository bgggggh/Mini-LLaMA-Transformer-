...
def run_multihead_self_attention_with_rope(
    ...
    rope_fn = lambda x, pos: rope(x, pos, theta)
    if token_positions is None:
        seq_len = in_features.shape[-2]
        token_positions = torch.arange(seq_len, device=in_features.device).unsqueeze(0)
    
    return multihead_self_attention(in_features, q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight, num_heads, rope_fn, token_positions)
...
def run_rope(
    ...
    return rope(in_query_or_key, token_positions, theta)
...
def run_get_batch(
    ...
    """
    # raise NotImplementedError
    from cs336_basics.model import get_batch
